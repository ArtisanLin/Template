                        ...
                        // e.g. 1 << 2
                            ...
                            /** @type {import('@babel/types').Expression | import('@babel/types').PrivateName} */ node) => {
                                ...
                                if (node.type === 'NumericLiteral' ||
                                    ...
                                    const exp = /** @type {`${string}.${string}`} */ (content.slice(node.start, node.end));
                                    if (!(exp in defines)) {
                            ...
                            if (init.argument.type === 'StringLiteral' ||
        ...
        transform(code, id) {
            ...
            if (id in enumData.declarations) {
                ...
                for (const declaration of enumData.declarations[id]) {
                    const { range: [start, end], id, members, } = declaration;
                    s.update(start, end, `export const ${id} = {${members
                        .flatMap(({ name, value }) => {
