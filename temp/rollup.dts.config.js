...
if (!existsSync('temp/packages')) {
    console.warn('no temp dts files found. run `tsc -p tsconfig.build-browser.json && tsc -p tsconfig.build-node.json` first.');
    process.exit(1);
}
            ...
            for (const node of ast.program.body) {
                if (node.type === 'VariableDeclaration') {
                    ...
                    if (node.declarations.length > 1) {
                        ...
                        throw new Error(`unhandled declare const with more than one declarators:\n${code.slice(node.start, node.end)}`);
                    }
                }
